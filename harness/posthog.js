...
if (!POSTHOG_PERSONAL_API_KEY || !POSTHOG_PROJECT_ID) {
  throw new Error("Missing POSTHOG_PERSONAL_API_KEY or POSTHOG_PROJECT_ID in .env")
}

export async function getMetrics(windowHours = 48) {
  const since = new Date(Date.now() - windowHours * 60 * 60 * 1000).toISOString()

  const [pageViews, homeBioSocialClicks, footerSocialClicks, projectsNavClicks] = await Promise.all(
    [
...
async function queryCount(eventName, since, properties = {}) {
  const url = new URL(`/api/projects/${POSTHOG_PROJECT_ID}/events/`, POSTHOG_API_HOST)
  ...
  return results.filter((event) =>
    Object.entries(properties).every(([k, v]) => event.properties?.[k] === v),
  ).length
}
