...
function generateBrief(metrics, experiments) {
  ...
  const target =
    issues.find((i) => !recentElements.includes(i.element)) ?? issues[0];
...
async function run() {
  ...
  if (!metrics.page_views || metrics.page_views < MIN_SESSIONS) {
    log(
      `Only ${metrics.page_views ?? 0} page views (min: ${MIN_SESSIONS}). Skipping cycle.`
    );
    return;
  }
  ...
  if (pending) {
    ...
    if (hoursSinceDeploy >= 24 && metrics.page_views >= (pending.min_exposure_views ?? MIN_SESSIONS)) {
  ...
  try {
    execSync(
      `claude --model claude-sonnet-4-6 --max-turns 25 -p ${JSON.stringify(prompt)}`,
      {
        cwd: worktreePath,
        stdio: "inherit",
        timeout: 8 * 60 * 1000,
  ...
  const disallowed = changedFiles.filter(
    (f) => !state.editable_files.includes(f)
  );
